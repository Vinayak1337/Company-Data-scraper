...
def is_probable_job(title: str, href: str) -> bool:
    ...
    if clean_title in GENERIC_NAV_TITLES:
        return False
    lowered = f"{title} {href}".lower()
    noisy = ("privacy", "cookie", "terms", "login", "signup", "blog", "press")
    if any(term in lowered for term in noisy):
        return False
    title_has_role_term = any(re.search(rf"\b{re.escape(term)}\b", clean_title) for term in JOB_TITLE_TERMS)
    path = urlparse(href).path.lower()
    path_has_job_marker = any(marker in path for marker in ("/jobs/", "/job/", "/positions/", "/openings/", "/listing/"))
    path_has_identifier = bool(re.search(r"/(?:[a-f0-9-]{8,}|\d{5,})(?:/|$)", path))
    return title_has_role_term or (path_has_job_marker and path_has_identifier)
